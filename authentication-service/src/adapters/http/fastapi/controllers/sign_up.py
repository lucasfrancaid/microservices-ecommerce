...
async def get_all_users(page: int = 1, limit: int = 20, factory: ApplicationFactory = Depends(factory_application)):
    response = [UserSchemaDTO(**user.__dict__) for user in await factory.repository.all(page=page, limit=limit)]
    return response
...
async def confirmation_account(
    ...
    try:
        response = await controller.put(
            repository=factory.repository, email_service=factory.email_service, entity=entity
        )
